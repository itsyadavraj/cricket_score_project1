...
void record(int m, int n) {
	pl3.max_run = 0, pl4.max_w = 0, pl3.max_four = 0, pl3.max_six = 0;
	int max_six = 0, max_four = 0, max_run = 0, max_wicket = 0;
	int i;
	for (i = 0; i < m; i++)
	{
	...
	printf("Highest runs scored by the batsman %d by %s\n", pl1[max_run].runs,
	       pl1[max_run].name);

	printf("Maximum fours scored by the batsman %d by %s\n", pl1[max_four].fours,
	       pl1[max_four].name);

	printf("Maximum sixes scored by the batsman %d by %s\n", pl1[max_six].sixes,
	       pl1[max_six].name);
	printf("Maximum wickets taken by the bowler %d by %s\n", pl2[max_wicket].wkttkn,
	       pl2[max_wicket].name);

}

void batsmanRecords(int plno) {
...
void bowlerRecords(int plno, int n) {
	plno--;
	int i;
	printf("                         Player Detail\n  ");
	printf("=================================================================\n");
